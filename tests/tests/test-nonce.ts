...
describeWithMoonbeam("Moonbeam RPC (Nonce)", `simple-specs.json`, (context) => {
  ...
  it("nonce should not be reset to 0 when emptying dust accounts", async function () {
    ...
    await context.polkadotApi.tx.balances.transfer(testAccount1.address, fee+1).signAndSend(genesisAccount);
    await createAndFinalizeBlock(context.polkadotApi);

    await context.polkadotApi.tx.balances.transfer(testAccount2.address, 1).signAndSend(testAccount1);
    await createAndFinalizeBlock(context.polkadotApi);

    const { nonce, data: balance } = await context.polkadotApi.query.system.account(testAccount1.address);
